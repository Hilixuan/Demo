...
(function () {
  ...
  /* ===== 文书智写 PRD 需求标注 ===== */
  function prdViewLabel(view) {
    return ({ projects: "项目管理", files: "企业资料库", writer: "文书智写", tune: "AI调优", score: "智能评分" })[view] || view;
  }

  function prdDotHtml(item) {
    return '<button class="prd-dot ' + (item.type === "agent" ? "prd-dot--agent" : "prd-dot--feature") + '" data-action="prd-open" data-id="' + esc(item.id) + '" title="' + esc(item.title) + '" aria-label="PRD ' + item.num + ' ' + esc(item.title) + '">' + item.num + "</button>";
  }

  function prdDotsFor(attach) {
    return '<span class="prd-dots">' + GQ.prdDocs.filter(d => d.attach === attach).map(prdDotHtml).join("") + "</span>";
  }

  function prdLegend() {
    return '<span class="prd-legend">' +
      '<span class="prd-legend-item"><i class="prd-dot-demo prd-dot-demo--feature"></i>功能需求</span>' +
      '<span class="prd-legend-item"><i class="prd-dot-demo prd-dot-demo--agent"></i>Agent 需求</span></span>';
  }

  function prdBar(view) {
    const items = GQ.prdDocs.filter(d => d.view === view);
    return '<div class="prd-bar"><span class="prd-bar-title">PRD 需求标注</span>' + prdLegend() +
      '<span class="prd-bar-dots">' + items.map(prdDotHtml).join("") + "</span>" +
      '<button class="btn btn-outline btn-sm prd-bar-more" data-action="prd-overview">' + icon("clipboard") + "全部 PRD</button></div>";
  }
  ...
  function viewApplicationWorkspace() {
    ...
    return '<div class="app-workspace-head">' +
      ...
      '<div class="prd-workspace-meta">' + prdLegend() + '<button class="btn btn-outline btn-sm" data-action="prd-overview">' + icon("clipboard") + "全部 PRD</button></div>" +
  ...
  function viewEnterpriseFiles(app) {
    ...
    return '<div class="grid-2-1 app-section">' +
      '<div class="card"><div class="card-head"><div><div class="card-title">企业文件资料</div><div class="card-sub">资料已按申报附件、设备证据、淘汰设备证明等目录分类展示</div></div>' + prdDotsFor("files-main") +
      ...
      '<div class="card ai-side-panel"><div class="card-head"><div><div class="card-title">Agent 资料识别</div><div class="card-sub">缺失资料、逻辑矛盾与证据链检测</div></div>' + prdDotsFor("files-ai") + st(state.aiScanDone ? "识别完成" : "待识别") + '</div>' +
  ...
  function viewSmartWriter(app) {
    ...
    return '<div class="grid-1-2 app-section writer-layout">' +
      '<div class="card"><div class="card-head"><div><div class="card-title">两新文书一级目录</div><div class="card-sub">根据参考模板提炼为演示大纲，逐章检查资料完整度</div></div>' + prdDotsFor("writer-outline") + '</div>' +
      ...
      '<div class="card writer-doc-card"><div class="card-head"><div><div class="card-title">文书主体内容 ' + aiDocChip() + '</div><div class="card-sub">' + esc(app.shortName) + ' · 国债两新申报书 · ' + (state.docGenerated ? "最新草稿实时缓存" : "待生成") + '</div></div>' + prdDotsFor("writer-doc") + docTopActions(readonly) + '</div>' +
  ...
  function viewAiTuneV2(app) {
    ...
    return '<div class="tune-workbench app-section"><div class="card"><div class="card-head"><div><div class="card-title">当前企业文书草稿 ' + aiDocChip() + '</div><div class="card-sub">读取文书智写最新草稿，支持人工手动调整</div></div>' + prdDotsFor("tune-main") + docTopActions(readonly) + '</div><div class="ai-line"><span></span><b>Agent 正在扫描段落相似度、企业特色缺口和通用化表达</b></div>' + docEditorHtml(readonly) +
      ...
      '<div class="tune-side"><div class="card"><div class="card-head"><div><div class="card-title">项目库横向对比</div><div class="card-sub">同机构文书交叉验证，提示雷同风险</div></div>' + prdDotsFor("tune-compare") + '<div class="ai-orbit tiny"><b>AI</b></div></div>' + compare + '</div></div></div>';
  }
  ...
  function viewSmartScoreV2(app) {
    ...
    return '<div class="score-workbench app-section"><div class="card writer-doc-card"><div class="card-head"><div><div class="card-title">文书终稿预览 ' + aiDocChip() + '</div><div class="card-sub">用于评分和全文校对的最新草稿</div></div>' + prdDotsFor("score-main") + docTopActions(readonly) + '</div><div class="doc-preview app-doc-preview score-doc-preview">' +
      ...
      '<div class="score-side"><div class="card score-ai-card ' + (state.scoreCollapsed ? "collapsed" : "") + '"><div class="card-head"><div><div class="card-title">AI评分</div><div class="card-sub">可收起查看，综合得分 88</div></div>' + prdDotsFor("score-ai") + '<button class="btn btn-outline btn-sm" data-action="app-toggle-score">' + (state.scoreCollapsed ? "展开" : "收起") + '</button></div>' +
      ...
      '<div class="card score-proof-panel"><div class="card-head"><div><div class="card-title">AI校对</div><div class="card-sub">建议修改 ' + proof.length + ' 条 · 固定高度滚动</div></div>' + prdDotsFor("score-proof") + '<div class="ai-orbit tiny"><b>AI</b></div></div><div class="ai-line"><span></span><b>校对光标正在锁定逻辑错误、前后矛盾和数据口径风险</b></div><div class="proof-scroll">' + proofHtml + '</div></div></div></div>';
  }
  ...
  function handleAction(action, el) {
    switch (action) {
      ...
      case "app-new-project-save":
        ...
        break;
      case "prd-open": {
        const item = GQ.prdDocs.find(d => d.id === el.dataset.id);
        if (!item) break;
        const typeBadge = item.type === "agent"
          ? '<span class="prd-type prd-type--agent"><i class="prd-dot-demo prd-dot-demo--agent"></i>Agent 需求</span>'
          : '<span class="prd-type prd-type--feature"><i class="prd-dot-demo prd-dot-demo--feature"></i>功能需求</span>';
        drawer("PRD 需求详情 · " + item.num, "文书智写 · " + prdViewLabel(item.view),
          '<div class="prd-detail-head">' + prdDotHtml(item) + '<div><div class="prd-detail-title">' + esc(item.title) + '</div><div class="prd-detail-meta">' + typeBadge + '<span>' + esc(item.priority || "P1") + '</span><span>' + esc(prdViewLabel(item.view)) + '</span></div></div></div>' +
          '<div class="qa-block" style="margin-top:16px"><div class="qa-block-title">需求描述</div><div class="qa-text">' + esc(item.desc) + '</div></div>' +
          '<div class="qa-block" style="margin-top:14px"><div class="qa-block-title">验收口径</div>' + (item.accept || []).map(a => '<div class="prd-accept"><i class="prd-accept-check">' + icon("check") + '</i><span>' + esc(a) + '</span></div>').join("") + '</div>' +
          (item.agent ? '<div class="qa-block" style="margin-top:14px"><div class="qa-block-title">关联 Agent</div><div class="qa-text">' + esc(item.agent) + '</div></div>' : ""),
          '<button class="btn btn-outline" data-action="prd-overview">' + icon("clipboard") + "全部 PRD</button>" +
          '<button class="btn btn-outline" data-close="drawer">关闭</button>');
        break;
      }
      case "prd-overview": {
        const groups = ["projects", "files", "writer", "tune", "score"];
        const body = groups.map(view => {
          const items = GQ.prdDocs.filter(d => d.view === view);
          if (!items.length) return "";
          return '<div class="prd-overview-group"><div class="prd-overview-group-title">' + esc(prdViewLabel(view)) + '</div>' +
            items.map(d => '<div class="prd-overview-row" data-action="prd-open" data-id="' + d.id + '">' + prdDotHtml(d) +
            '<div><b>' + esc(d.title) + '</b><span>' + esc(d.desc) + '</span></div>' +
            (d.type === "agent" ? '<span class="prd-type prd-type--agent">Agent</span>' : '<span class="prd-type prd-type--feature">功能</span>') + '</div>').join("") + '</div>';
        }).join("");
        drawer("文书智写 · PRD 需求清单", "共 " + GQ.prdDocs.length + " 条需求，蓝色为功能需求，橘黄色为 Agent 需求",
          body,
          '<button class="btn btn-outline" data-close="drawer">关闭</button>');
        break;
      }
